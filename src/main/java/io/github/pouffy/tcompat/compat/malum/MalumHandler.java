...
public class MalumHandler {
    ...
    public static Projectile createHexBolt(LivingEntity shooter, Vec3 pos, float pitchOffset, int spawnDelay, float velocity, float magicDamage) {
        ...
        return LoadedOnly.createHexBolt(shooter, pos, pitchOffset, spawnDelay, velocity, magicDamage);
    }
    ...
    public static Projectile createErosion(LivingEntity shooter, Vec3 pos, float pitchOffset, int spawnDelay, float velocity, float magicDamage) {
        if (!CompatHelper.isLoaded("malum")) return null;
        return LoadedOnly.createErosion(shooter, pos, pitchOffset, spawnDelay, velocity, magicDamage);
    }
    ...
    public static Pair<Supplier<MobEffect>, Predicate<LivingEntity>> getRiteEffect(String name, boolean corrupted) {
        Supplier<MobEffect> effect = () -> null; Predicate<LivingEntity> predicate = (e) -> false;
        if (!CompatHelper.isLoaded("malum")) new Pair<>(effect, predicate);
        return LoadedOnly.getRiteEffect(name, corrupted);
    }

    public static void idleRestoration(LivingEvent.LivingTickEvent event) {
        // No point running if the player has the actual rune
        if (EquipmentHelper.hasModifier(event.getEntity(), TCModifiers.runeOfIdleRestoration) && !CuriosHandler.hasItem(event.getEntity(), "malum:rune_of_idle_restoration")) {
            if (event.getEntity().level().getGameTime() % 40L == 0L) {
    ...
    public static void volatileDistortion(LivingHurtEvent event, LivingEntity attacker) {
        // No point running if the player has the actual rune
        if (EquipmentHelper.hasModifier(attacker, TCModifiers.runeOfVolatileDistortion) && !CuriosHandler.hasItem(attacker, "malum:rune_of_volatile_distortion")) {
    ...
    public static void reactiveShielding(LivingHurtEvent event, LivingEntity attacker) {
        // No point running if the player has the actual rune
        if (CompatHelper.isLoaded("malum") && EquipmentHelper.hasModifier(attacker, TCModifiers.runeOfReactiveShielding) && !CuriosHandler.hasItem(attacker, "malum:rune_of_reactive_shielding")) {
            LoadedOnly.reactiveShielding(event);
        }
    }

    public static void ailmentCleansing(MobEffectEvent.Added event) {
        // No point running if the player has the actual rune
        if (CompatHelper.isLoaded("malum") && EquipmentHelper.hasModifier(event.getEntity(), TCModifiers.runeOfAlimentCleansing) && !CuriosHandler.hasItem(event.getEntity(), "malum:rune_of_aliment_cleansing")) {
            LoadedOnly.ailmentCleansing(event);
        }
    }
    ...
    public static class LoadedOnly {
        ...
        public static boolean staffReserve(Player player) {
            MalumPlayerDataCapability capability = MalumPlayerDataCapability.getCapability(player);
            int cachedCharge = capability.reserveStaffChargeHandler.chargeCount;
            if (cachedCharge > 0) {
                --capability.reserveStaffChargeHandler.chargeCount;
            }
            return cachedCharge > 0;
        }
        ...
        public static void reactiveShielding(LivingHurtEvent event) {
