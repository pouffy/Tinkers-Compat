...
public class AuricFlameModifier extends MalumStaffModifier {
    ...
    public void spawnChargeParticles(Level level, LivingEntity entity, Vec3 pos, IToolStackView tool, float chargePercentage) {
        ...
        MalumClientHandler.auricFlameParticles(entity, random, pos, chargePercentage);
    }
    ...
    public void fireProjectile(LivingEntity entity, IToolStackView tool, Level level, InteractionHand hand, float chargePercentage, int count) {
        ...
        var auricFlame = MalumHandler.createAuricFlame(entity, pos, pitchOffset, spawnDelay, velocity, (float) magicDamage, spread);
        if (auricFlame != null) {
            ProjectileAbilityHooks.addModifiersToProjectile(tool, entity, auricFlame, count < (getProjectileCount(level, entity, chargePercentage) / 2));
            level.addFreshEntity(auricFlame);
        }
    }
}
